(() => {
  ...
  const messages = isEnglish ? {
    ...
    success: 'Thank you. Your testimonial has been received and is awaiting review. Nothing has been published automatically.',
    unavailable: 'The testimonial service is not configured yet. Please try again later.',
    error: 'The testimonial could not be sent. Please check your connection and try again.',
    invalid: 'Please check the required fields before submitting.'
  } : {
    sending: 'Envoi en cours…',
    submit: 'Envoyer mon témoignage pour validation',
    success: 'Merci. Votre témoignage a bien été reçu et attend maintenant une validation. Rien n’a été publié automatiquement.',
    unavailable: 'Le service de témoignages n’est pas encore configuré. Merci de réessayer ultérieurement.',
    error: 'Le témoignage n’a pas pu être envoyé. Vérifiez votre connexion puis réessayez.',
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      verification_contact: String(data.get('verification_contact') || '').trim(),
      collaboration_context: String(data.get('collaboration_context') || '').trim(),
