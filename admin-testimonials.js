(() => {
  ...
  const isConfigured = Boolean(apiUrl && anonKey && !apiUrl.includes('YOUR_') && !anonKey.includes('YOUR_'));
  ...
  const restoreSession = () => {
    try {
      ...
      if (value?.access_token && value.expires_at > Date.now() + 60000) session = value;
  ...
  const moderate = async (id, nextStatus, name) => {
    ...
    try {
      ...
      const response = await request(`/rest/v1/testimonials?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
      ...
      setText(moderationStatus, nextStatus === 'approved'
        ? `Le témoignage de ${name} est maintenant publié.`
        : `Le témoignage de ${name} a été refusé.`, 'success');
      await loadRows();
    } catch {
      setText(moderationStatus, 'La décision n’a pas pu être enregistrée. Vérifiez vos droits puis réessayez.', 'error');
    }
  };
  ...
  const renderRows = (rows) => {
    ...
    rows.forEach((row) => {
      ...
      data.append(
        ...
        createRow('Identification publique', identityLabels[row.identity_mode] || row.identity_mode),
        createRow('Points mis en avant', Array.isArray(row.strengths) ? row.strengths.join(', ') : '')
      );
      ...
      if (row.status !== 'approved') {
        ...
        approve.setAttribute('aria-label', `Publier le témoignage de ${row.full_name}`);
        approve.addEventListener('click', () => moderate(row.id, 'approved', row.full_name));
      ...
      if (row.status !== 'rejected') {
        ...
        reject.textContent = row.status === 'approved' ? 'Retirer du site' : 'Refuser';
        reject.setAttribute('aria-label', row.status === 'approved'
          ? `Retirer du site le témoignage de ${row.full_name}`
          : `Refuser le témoignage de ${row.full_name}`);
        reject.addEventListener('click', () => moderate(row.id, 'rejected', row.full_name));
  ...
  const loadRows = async () => {
    ...
    const fields = 'id,full_name,role,organization,verification_contact,collaboration_context,testimonial,strengths,identity_mode,consent,status,submission_language,created_at,approved_at';
    try {
      const response = await request(`/rest/v1/testimonials?select=${fields}&status=eq.${activeStatus}&order=created_at.asc`, {}, true);
      ...
      renderRows(rows);
      setText(moderationStatus, `${rows.length} témoignage${rows.length > 1 ? 's' : ''} dans cette catégorie.`);
    } catch {
      setText(moderationStatus, 'Impossible de charger les témoignages.', 'error');
    }
  };
  ...
  const showDashboard = () => {
    ...
    identity.textContent = session?.email ? `Connectée : ${session.email}` : 'Session administratrice active';
    loadRows();
  };
  ...
  loginForm?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      storeSession({
        ...
        expires_at: Date.now() + Math.max(0, Number(auth.expires_in || 3600) - 30) * 1000
      });
      loginForm.reset();
      setText(loginStatus, '');
      showDashboard();
    } catch {
      setText(loginStatus, 'Connexion impossible. Vérifiez l’adresse, le mot de passe et le rôle moderator.', 'error');
    } finally {
