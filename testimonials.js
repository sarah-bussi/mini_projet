(() => {
  ...
  const copy = isEnglish ? {
    ...
    loaded: (count) => `${count} approved testimonial${count === 1 ? '' : 's'} loaded.`,
    ...
    loaded: (count) => `${count} témoignage${count > 1 ? 's' : ''} validé${count > 1 ? 's' : ''} chargé${count > 1 ? 's' : ''}.`,
  ...
  const loadTestimonials = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/rest/v1/rpc/get_published_testimonials`, {
        method: 'POST',
        headers: {
          apikey: anonKey,
